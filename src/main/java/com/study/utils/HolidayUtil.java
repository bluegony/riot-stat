...
public class HolidayUtil {
    ...
    private static String[] lunarArr = new String[]{"1230", "0101", "0102", "0408", "0814", "0815", "0816"};
    private static List<String> holidayDB;
    ...
    public boolean isHoliday(String date) {
        try {
            return isHolidayDatabase(date) || isHolidaySolar(date) || isHolidayLunar(date) || isHolidayAlternate(date) || isWeekend(date);
    ...
    private boolean isHolidayDatabase(String date) {
        if(holidayDB==null) {
            holidayDB = propertyMapper.selectPropertyListByKey("HOLIDAY_SOLAR");
            for(String a:holidayDB) {
                log.info("holiday DB:{}",a);
            }
        }
        return holidayDB.contains(date);
    }
